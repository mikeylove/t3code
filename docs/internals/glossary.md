...
| Author         | The person a user message or prompt answer came from, stamped by the server from the paired session.  |
| Thread guest   | A session paired from a thread invite. It carries ordinary scopes but is confined to that one thread. |

...
const ACCESS_ROW_BADGE_CLASSNAME =
  "text-[10px] text-muted-foreground/80 rounded-md border border-border/50 bg-muted/50 px-1 py-0.5";

/** Marks a pairing link or session restricted to one thread, naming the thread when the shell has it. */
function ThreadScopeBadge({
  threadId,
  kind,
}: {
  readonly threadId: ThreadId;
  readonly kind: "invite" | "guest";
}) {
  const primaryEnvironmentId = usePrimaryEnvironmentId();
  const thread = useThreadShell(
    primaryEnvironmentId === null ? null : scopeThreadRef(primaryEnvironmentId, threadId),
  );
  const label = kind === "invite" ? "Thread invite" : "Thread guest";
  return (
    <span className={cn(ACCESS_ROW_BADGE_CLASSNAME, "min-w-0 truncate")}>
      {thread ? `${label} · ${thread.title}` : label}
    </span>
  );
}
      ...
      <div className={ITEM_ROW_INNER_CLASSNAME}>
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex min-h-5 items-center gap-1.5">
            ...
            {pairingLink.threadId !== undefined ? (
              <ThreadScopeBadge threadId={pairingLink.threadId} kind="invite" />
            ) : null}
          </div>
        ...
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex min-h-5 items-center gap-1.5">
            ...
            {clientSession.current ? (
              <span className={ACCESS_ROW_BADGE_CLASSNAME}>This device</span>
            ) : null}
            {clientSession.threadId !== undefined ? (
              <ThreadScopeBadge threadId={clientSession.threadId} kind="guest" />
            ) : null}
          </div>
          <p className="text-xs text-muted-foreground">

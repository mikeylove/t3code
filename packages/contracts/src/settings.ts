...
export const ServerSettings = Schema.Struct({
  ...
  /**
   * How the environment owner is named on the messages they send, shown to
   * other participants and to the agent in shared threads. Chosen by the
   * person, never derived from the login account. Null until they set it.
   */
  ownerDisplayName: Schema.NullOr(TrimmedNonEmptyString).pipe(
    Schema.withDecodingDefault(Effect.succeed(null)),
  ),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  ownerDisplayName: Schema.optionalKey(Schema.NullOr(TrimmedNonEmptyString)),
  continueThreadsAfterServerUpdate: Schema.optionalKey(Schema.Boolean),
  enableAgentBrowserAccess: Schema.optionalKey(Schema.Boolean),
